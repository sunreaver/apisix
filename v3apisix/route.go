...
// https://apisix.apache.org/zh/docs/apisix/3.10/admin-api/#route-request-body-parameters
type Route struct {
	...
	Timeout     *UpstreamTimeout `json:"timeout,omitempty"`      // 为 Route 设置 Upstream 连接、发送消息和接收消息的超时时间（单位为秒）。该配置将会覆盖在 Upstream 中配置的 timeout 选项。
