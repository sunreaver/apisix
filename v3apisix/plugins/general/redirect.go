...
type Redirect struct {
	...
	RetCode           int    `json:"ret_code"`                // HTTP 响应码; 默认302
	EncodeUri         bool   `json:"encode_uri"`              // 当设置为 true 时，对返回的 Location Header 按照 RFC3986 的编码格式进行编码。
	AppendQueryString bool   `json:"append_query_string"`     // 当设置为 true 时，将原始请求中的查询字符串添加到 Location Header。如果已配置 uri 或 regex_uri 已经包含查询字符串，则请求中的查询字符串将附加一个&。如果你已经处理过查询字符串（例如，使用 NGINX 变量 $request_uri），请不要再使用该参数以避免重复。
}
